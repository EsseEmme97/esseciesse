	...
	public function get_node($id, $options = array()) {
		$node = $this->db->one("
			...
			WHERE 
				...
				s.".$this->options['structure']['id']." = ".(int)$id." AND
				d.id_lingua=1" // x cambiare lingua principale
		);
		if(!$node) {
		...
		if($recursive) {
			...
			$sql = "
				...
				WHERE 
					...
					s.".$this->options['structure']['right']." < ".(int)$node[$this->options['structure']['right']]." AND
					d.id_lingua=1
				ORDER BY 
			...
			$sql = "
				...
				WHERE 
					s.".$this->options['structure']['id']." = d.".$this->options['data2structure']." AND 
					s.".$this->options['structure']['parent_id']." = ".(int)$id." AND
					d.id_lingua=1
				ORDER BY 
