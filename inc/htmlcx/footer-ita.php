<!-- start footer -->
<footer class="p-0 footer-light position-relative bg-light-gray">
    <div class="container position-relative">
        <div class="row pt-5 sm-pt-40px justify-content-between">
            <!-- start footer column -->
            <div class="col-6 col-xl-6 col-lg-12 col-sm-6 last-paragraph-no-margin text-xl-start text-lg-center order-sm-1 lg-mb-50px sm-mb-30px">
                <a href="demo-it-business.html" class="footer-logo mb-15px d-inline-block">
                    <img src="<?= $pathindex ?>assets/images/global/logo/SCS-logo.svg" data-at2x="images/demo-it-business-logo-black@2x.png" alt="">
                </a>
            </div>
            <!-- end footer column -->
            <!-- start footer column -->
            <div class="col-6 col-xl-1 col-lg-3 col-sm-4 xs-mb-30px order-sm-3 order-lg-2">
                <a href="#" class="fw-600 d-block text-dark-gray mb-5px">Home</a>
            </div>
            <!-- end footer column -->
            <!-- start footer column -->
            <div class="col-6 col-xl-1 col-lg-3 col-md-5 col-sm-4 xs-mb-30px order-sm-4 order-lg-3">
                <a href="#" class="fw-600 d-block text-dark-gray mb-5px">Su di noi</a>
            </div>
            <!-- end footer column -->
            <!-- start footer column -->
            <div class="col-6 col-xl-1 col-lg-3 col-md-3 col-sm-4 xs-mb-30px order-sm-5 order-lg-4">
                <a href="#" class="fw-600 d-block text-dark-gray mb-5px">Lavori</a>
            </div>
            <!-- end footer column -->
            <!-- start footer column -->
            <div class="col-xl-1 col-lg-3 col-sm-6 md-mb-50px sm-mb-30px xs-mb-0 order-sm-2 order-lg-5">
                <a href="#" class="fw-600 d-block text-dark-gray mb-5px">Contatti</a>
            </div>
            <!-- end footer column -->
        </div>
        <div class="row align-items-center">
            <!-- start divider -->
            <div class="col-12">
                <div class="divider-style-03 divider-style-03-01 border-color-extra-medium-gray"></div>
            </div>
            <!-- end divider -->
            <!-- start copyright -->
            <div class="col-lg-7 pt-20px pb-20px md-pt-0 order-2 order-lg-1 text-center text-lg-start last-paragraph-no-margin fs-14">
                <p class="h5">&copy; <?= date("Y") ?> Esseciesse srl | <a href="#" target="_blank">Privacy Policy</a> | By <a href="https://carbonx.it/">Carbonx</a></p>
                <p class="h5">C.F e P.IVA e Reg.Imp 04078920248 | Rea VI - 377141 | Capitale sociale € i.v. 150.000,00</p>
            </div>
            <!-- end copyright -->
            <!-- start footer menu -->
            <div class="col-lg-5 pb-20px md-pt-25px md-pb-5px order-1 order-lg-2 text-center text-lg-end fs-14">
                <a href="#">
                    <i class="bi bi-linkedin fs-24 text-orange"></i>
                </a>
            </div>
            <!-- end footer menu -->
        </div>
    </div>
</footer>
<!-- end footer -->